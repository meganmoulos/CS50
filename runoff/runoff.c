...
bool print_winner(void)
{
    ...
    // if nobody has won the election yet, print nothing and return false
    return false;
}
...
int find_min(void)
{
    // Find the fewest number of votes if they aren't eliminated yet
    int min_votes;
    for (int i = 0; i < candidate_count; i++)
    {
...
bool is_tie(int min)
{
    // return true if election tie between all remaining candidates
    for (int i = 0; i < candidate_count; i++)
    {
...
void eliminate(int min)
{
    // Eliminate anyone still in the race who has min number of votes.
    for (int i = 0; i < candidate_count; i++)
    {
        if (candidates[i].eliminated == false && candidates[i].votes == min)
        {
